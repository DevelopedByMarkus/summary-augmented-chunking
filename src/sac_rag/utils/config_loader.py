...
from sac_rag.methods.hypa import HypaStrategy

# A type hint for any of our possible strategy models
AnyRetrievalStrategy = Union[BaselineStrategy, HypaStrategy]
...
def load_strategy_from_file(filepath: str) -> AnyRetrievalStrategy:
    ...
    # Add 'method_name' to data if it's HypaStrategy, which requires it
    if strategy_type == "hypa":
        model = HypaStrategy
        if "method_name" not in data:
            data["method_name"] = "hypa" # Default value
    elif strategy_type == "baseline":
        model = BaselineStrategy
    else:
        raise ValueError(
            f"Invalid 'strategy_type' in {filepath}: '{strategy_type}'. "
            "Must be 'baseline' or 'hypa'."
        )
